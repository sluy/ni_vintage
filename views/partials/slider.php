...
foreach (array_reverse($ctx['service']->posts) as $current) {
  ...
  $html = '<div class="ls-l post" ' .
    'style="' . $slideConfig[$imagePostCounter]['style'] . 'width:' . $current->width . ';height:' . $current->height . ';" ' +
    'data-ls="' . str_replace('__DELAY__', $delay, $slideConfig[$imagePostCounter]['data-ls']) . '">' .
    '<div class="content">';
  if ($current->src) {
  ...
  array_unshift($htmlContent, $html);
}
